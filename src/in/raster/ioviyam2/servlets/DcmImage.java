...
public class DcmImage extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (isRID == null) {
			imageURL = serverURL + "?requestType=WADO&studyUID=" + study
					+ "&seriesUID=" + series + "&objectUID=" + object;
			...
			imageURL += "/rid/IHERetrieveDocument?requestType=DOCUMENT&documentUID="
					+ object + "&preferredContentType=application/pdf";
		}
		synchronized (this) {
			InputStream is = null;
			OutputStream os = null;

			// Get the response OutputStream instance to write a image in the
			// response.
			if (isgzip) {
			...
			try {
				...
				if (object != null && object.length() > 0) {
					...
					// Initialize the byte array to read and hold the bytes from
					// the
					// imgURL stream.
				...
				log.error("Unable to read and write the image from "
						+ serverURL, e);
			} finally {
